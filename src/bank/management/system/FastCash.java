         ...
         if(e.getSource()==exit){
             ...
             new Transaction(pinnumber).setVisible(true);
              ...
              String amount = ((JButton) e.getSource()).getText().substring(3);
                Conn conn = new Conn();
                try{ 
                    ResultSet res = conn.s.executeQuery("select * from bank where pin = '"+pinnumber+"'");
                    int balance =0;
                    while(res.next()){
                    ...
                    if(balance <Integer.parseInt(amount)){
                        JOptionPane.showMessageDialog(null,"Insufficient Balnce Your Balance is Rs:"+balance);
                        return;
                    }
                    Date date = new Date();
